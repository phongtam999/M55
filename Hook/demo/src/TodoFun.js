...
function TodoFun(props) {
  ...
  const updateTodo = () => {
    ...
    const addButton = <button onClick={() => addTodo()}> Thêm </button>;
    const updateButton = <button onClick={() => updateTodo()}> Cập nhật </button>;
  return (
    <div>
      <h1> {todo}</h1>
      <input
        ...
        <tbody>
          {todos.map((todo, key) => (
            <tr key={key}>
              <td>{todo}</td>
              <td>
                <button onClick={() => editTodo(key)}> Sửa </button>||
                <button onClick={() => deleteTodo(key)}> Xoá </button>
